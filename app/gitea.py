...
from utils import send_message, build_content
...
async def gitea_webhook(request: Request, payload: GiteaPayload):
    ...
    store_data(uuid, number, action, merged, state, repo_name, username, url, title)
    logger.info("收到pr")
    ...
    content = build_content(uuid, action, merged, state, repo_name, username, url, title)
